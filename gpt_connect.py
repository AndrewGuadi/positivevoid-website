...
def filter_user_input(text):
    ...
    example = "{'status': pass_or_fail, 'status_reason':'only_if_failed_reason_why_failed', 'text': *input_text_goes_here*, 'emotions': [array of emotions perceived in text], 'sentiment_basic': positive_neutral_negative, 'sentiment_score': on_scale_from_-100_to_+100, 'TLDR': point_of_message, 'keywords': [array of 3-10 keywords here], 'challenging_thought':'this_should_be_a_positive_unique_yet_very_insightul_wisdom_that_directly_or_tangentially_relates(use REAL quotes sporadically)'}"
